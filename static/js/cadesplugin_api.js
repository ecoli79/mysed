; (function () {
    ...
    function load_js_script(url, successFunc, errorFunc) {
        ...
        // Улучшенная обработка ошибок - не показываем ошибку в консоли для несуществующих расширений
        script.onerror = function() {
            // Проверяем, является ли это ошибкой загрузки расширения
            // Если URL содержит 'invalid', это означает, что расширение не установлено
            // Это нормально, просто вызываем errorFunc без логирования ошибки
            if (errorFunc) {
                errorFunc();
            }
        };
    ...
    function load_extension() {
        ...
        // Пробуем сначала Manifest V3 (новое расширение), затем Manifest V2 (старое)
        load_js_script(manifestv3Url, nmcades_api_onload, function () {
            // Если Manifest V3 не загрузился, пробуем Manifest V2
            load_js_script(manifestv2Url, nmcades_api_onload, plugin_loaded_error);
        });
    }
